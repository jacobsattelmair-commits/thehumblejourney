...
async function whoopGet(path, token) {
  ...
  if (!res.ok) {
    const body = await res.text();
    throw new Error('Whoop API error ' + res.status + ' for ' + path + ': ' + body);
  }
  return res.json();
}
...
export default async function handler() {
  ...
  try {
    // Use Whoop API v2 endpoints
    const [recoveryData, sleepData, cycleData] = await Promise.all([
      whoopGet('/developer/v2/recovery?limit=1', access),
      whoopGet('/developer/v2/activity/sleep?limit=1', access),
      whoopGet('/developer/v2/cycle?limit=1', access),
    ]);
    ...
    if (sleepData.records && sleepData.records[0]) {
      ...
      entry.sleepDurationHrs = (s.end && s.start)
        ? Math.round((new Date(s.end) - new Date(s.start)) / 36e5 * 10) / 10
        : null;
    }
    ...
    return new Response('Synced Whoop data for ' + today + ': ' + JSON.stringify(entry), { status: 200 });
  } catch (err) {
